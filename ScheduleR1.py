...
default_disciplines = {
    "Civil": ["Civil", "GIS", "Geotechnical"],
    "Electrical": ["Electrical", "PV"],
    "HSE": ["HSE", "Safety"],
    "Instrument": ["Instrument", "SCADA", "I&C", "Automation"],
    "Management": ["Project Manager", "DCC", "Project Engineer", "Manager"],
    "Mechanical": ["Mechanical"],
    "Site Supervision": ["Supervisor", "Commissioning"],
    "Other": []
}
...
if st.sidebar.button("Add Discipline"):
    if new_discipline and new_keywords:
        st.session_state.disciplines[new_discipline] = [k.strip() for k in new_keywords.split(",")]
        st.sidebar.success(f"Added {new_discipline} with keywords: {new_keywords}")
    else:
...
for discipline, keywords in st.session_state.disciplines.items():
    st.sidebar.write(f"- **{discipline}**: {', '.join(keywords)}")
...
# Add this section after the file is uploaded and processed
if uploaded_file is not None:
    ...
    # =============================
    # (Rest of your existing code for Gantt charts and visualizations)

    # =============================
    # 7) Gantt Chart by "Service"
    # =============================
